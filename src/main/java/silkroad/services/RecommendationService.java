...
public class RecommendationService {
    ...
    public List<AuctionBrowsingDetails> recommend(Authentication authentication) {
        final Integer maximumRecommendations = 2;
    ...
    @Scheduled(timeUnit = TimeUnit.SECONDS, fixedDelay = 600)
    public void factorize() {
        List<String> sortedUsers = this.userRepository.findSortedUsernames();
        List<Long> sortedAuctions = this.auctionRepository.findSortedIds(TimeManager.now());
